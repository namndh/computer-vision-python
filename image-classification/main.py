...
batch_size = 100 #(number of records in each batch)
total_batches = 200 #(number of batch in each epoch)

x_input = tf.placeholder(tf.float32, shape=[None, input_size])
y_input = tf.placeholder(tf.float32, shape=[None, num_classes])

weights = tf.Variable(tf.random_normal([input_size, num_classes]))
